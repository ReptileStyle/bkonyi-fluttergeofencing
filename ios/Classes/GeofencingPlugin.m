...
monitoringDidFailForRegion:(CLRegion *)region
    ...
    // Forward the call to the custom location delegate
    if ([self.customLocationDelegate respondsToSelector:@selector(locationManager:monitoringDidFailForRegion:withError:)]) {
        [self.customLocationDelegate locationManager:manager monitoringDidFailForRegion:region withError:error];
    }
}
...
- (instancetype)init:(NSObject<FlutterPluginRegistrar> *)registrar {
    ...
    self.customLocationDelegate = self;
    return self;
}
...
- (void)startGeofencingService:(int64_t)handle {
    ...
    // Set the custom location delegate as the delegate for the location manager
    _locationManager.delegate = self.customLocationDelegate;
}
